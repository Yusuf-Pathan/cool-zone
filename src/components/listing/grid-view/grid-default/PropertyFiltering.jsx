...
export default function PropertyFiltering() {
  ...
  useEffect(() => {
    setPageItems(data.slice((pageNumber - 1) * 8, pageNumber * 8));
    setPageContentTrac([(pageNumber - 1) * 8 + 1, pageNumber * 8, data.length]);
  }, [pageNumber, data]);
  ...
  const handlepropertyTypes = (elm) => {
    console.log(elm, "- property type");
    // console.log(propertyTypes, '- prop types')
    if (elm == "All") {
      setPropertyTypes([]);
    } else {
  ...
  async function getData() {
    const result = await axios.get(
      "http://localhost:8090/realestate/properties"
    );
    return result;
  }

  useEffect(() => {
    const response = getData().then((result) => {
      if (result?.data?.message === "propety data getting sucessfully") {
        const { data } = result?.data;
        setData(data);
      } else {
        console.log("Didn't get property data.");
      }
    });
  }, []);
  ...
  const handleFilter = (filterType, filterValue) => {
    const tempData = pageItems;
    if (filterType === "propertyType") {
      const filterData = tempData.filter(
        (item) => item.propertyType === filterValue
      );
    }
  };
