...
const PropertyHeader = ({ id, property }) => {
  // const property = listings.filter((elm) => elm.id == id)[0] || listings[0];
  const propertyArea = property?.price / property?.pricePerSqFt;
  console.log(propertyArea, '- prop area')
  return (
    <>
        ...
        <div className="single-property-content mb30-md">
          ...
          <div className="property-meta d-flex align-items-center">
            ...
            <a className="text ml20 fz15" href="#">
              ...
              {property?.sqft ? `${property?.sqft}` : propertyArea} sqft
            </a>
          </div>
        </div>
      </div>
        ...
        <div className="single-property-content">
          <div className="property-action text-lg-end">
            ...
            {/* <p className="text space fz15">
              $
              {(
                Number(property?.price?.split("$")[1]?.split(",")?.join("")) /
                property?.sqft
              ).toFixed(2)}
              /sq ft
            </p> */}
